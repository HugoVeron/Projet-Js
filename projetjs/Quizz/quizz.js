...
let mauvaise_annee //on crée la variable mauvaise année au cas ou tous les medias datent de la meme année et que l'on doivent changer l'année
...
document.getElementById("commencer").addEventListener('click', () => {//on commence le quizz quand on appuie sur le bouton "commencer le quizz"
	...
	if (type_media == "video")//on pose la question relative à la vidéo(film)
	{
		...
		{
			mauvaise_annee=entierAleatoire(list_of_media[num_playlist][5]-3,list_of_media[num_playlist][5]);
		}
		...
		if(bonne_option==1)//choix pour placer les checkboxs
		{
			...
			if(document.getElementById("checkbox1").checked == true)//on actualise le score
			{
				bonnes_rep+=1
			}
		...
		//on regénère toutes les données àléatoires
	...
	else
	{
		...
		else
		{
			...
			if(document.getElementById("titre_input").value == list_of_media[num_playlist][1] )
				{
					bonnes_rep+=1
				}
...
document.getElementById("valider").addEventListener('click', () => {
	...
	if (nb_question<list_of_media.length+1)//on vérifie que le nombre de question ne dépasse pas la taille de la playlist( je n'ai pas eu le temps de faire en sorte )
	{
		...
		nb_question+=1;// on augmente nombre de question pour vérifier que l'on en fait pas trop 
	}
	else 
	{
